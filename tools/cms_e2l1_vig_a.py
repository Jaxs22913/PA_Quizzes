...
VIG_A = [
 dict(topic="Contact lens keratitis", io=IOA, lead="next step", slot="escalation",
   ...
   opts=[
     ...
     ["Dispense a topical anesthetic for comfort at home",
      "Take-home anesthetic delays healing and masks progression."],
 ...
 dict(topic="Contact lens keratitis", io=IOA, lead="education", slot="education",
   ...
   opts=[
     ["Return immediately if the pain increases, the white spot enlarges, or the vision drops further",
      "Correct — microbial keratitis can perforate within a day, so the patient must know which changes mean returning at once rather than waiting for the appointment."],
     ["Resume lens wear as soon as the pain settles",
      "Lenses must stay out; wear is the risk factor that caused this."],
     ["Use the anesthetic drops whenever the eye is uncomfortable",
      "Take-home anesthetic is prohibited."],
 ...
 dict(topic="Bacterial conjunctivitis", io=IOA, lead="first-line", slot="first-line",
   ...
   opts=[
     ...
     ["A topical histamine blocker with a mast cell stabilizer",
 ...
 dict(topic="Bacterial conjunctivitis", io=IOA, lead="next step", slot="referral",
   ...
   opts=[
     ["Urgent referral to ophthalmology",
      "Correct — lens wear plus failure to improve in 24 hours together suggest this is keratitis rather than conjunctivitis, and the cornea is at risk."],
     ...
     ["Switch to an oral antibiotic and review in 48 hours",
      "Escalating orally treats a presumed conjunctivitis harder, when the real question is whether the cornea is involved; that needs slit lamp examination."],
     ["Add a topical corticosteroid to settle the inflammation",
      "Steroids risk worsening an unrecognized keratitis."]],
   c=0, cite=c(41)),
 ...
 dict(topic="Chlamydial conjunctivitis", io=IOA, lead="education", slot="education",
   q="A 26-year-old woman is starting doxycycline for adult inclusion conjunctivitis. Which counseling point is emphasized?",
   opts=[
     ["Stay out of the sun, take it with a full glass of water, stay upright, and separate it from antacids and iron, calcium or magnesium",
      "Correct — photosensitivity plus the absorption and esophageal cautions."],
     ["Take it on an empty stomach and lie down for an hour afterwards",
      "Lying down risks esophageal irritation; the advice is to stay upright."],
 ...
 dict(topic="Chlamydial conjunctivitis", io=IOA, lead="next step", slot="referral",
   ...
   opts=[
     ...
     ["Repeat the conjunctival test in one week to confirm clearance",
      "A test of cure is not required; what matters is that ocular chlamydia in an adult reflects genital infection, so she and her partner need evaluation."],
 ...
 dict(topic="Viral conjunctivitis", io=IOA, lead="education", slot="education",
   ...
   opts=[
     ["It is contagious, so hand hygiene matters; expect it to worsen over the first week and resolve in two to three weeks",
      "Correct — adenoviral conjunctivitis is highly contagious by hand contact and characteristically worsens for several days before settling, so early deterioration is expected rather than treatment failure."],
     ...
     ["It is not contagious and needs no precautions",
      "Adenoviral conjunctivitis spreads readily on hands and shared surfaces, which is why hand hygiene and avoiding shared towels matter."],
 ...
 dict(topic="Viral conjunctivitis", io=IOA, lead="next step", slot="referral",
   ...
   opts=[
     ["Refer to ophthalmology", "Correct — photophobia and reduced vision mean the cornea is involved rather than the conjunctiva alone, and four weeks places this outside the expected course."],
 ...
 dict(topic="Allergic conjunctivitis", io=IOA, lead="first-line", slot="agent/regimen",
   q="A 17-year-old has bilateral itchy red eyes every spring with swollen lids and stringy watery discharge. Vision is normal. Which single topical agent both blocks histamine and stabilizes mast cells?",
   opts=[
     ["Olopatadine", "Correct — olopatadine antagonizes the histamine receptor for immediate relief while stabilizing mast cells to prevent further release, covering both phases of the allergic response."],
     ["A topical fluoroquinolone", "That is an antibiotic and treats nothing here."],
     ["A topical corticosteroid", "Topical corticosteroids control allergic inflammation but carry pressure and cataract risk with repeated seasonal use, so they are not first-line."],
 ...
 dict(topic="Episcleritis", io=IOA, lead="initial test", slot="gold standard",
   ...
   opts=[
     ["Instill 2.5 percent phenylephrine and look for blanching of the vessels after 15 minutes",
      "Correct — episcleral vessels blanch; the mild painless sectoral redness fits episcleritis."],
     ["Instill fluorescein and examine under cobalt blue light for a dendrite",
 ...
 dict(topic="Episcleritis", io=IOA, lead="first-line", slot="first-line",
   ...
   opts=[
     ["Artificial tears and an oral non-steroidal anti-inflammatory taken with food",
      "Correct — episcleritis is self-limiting, so lubrication plus an oral non-steroidal anti-inflammatory suffices, taken with food because these agents irritate the gastric mucosa."],
     ["Systemic corticosteroids started immediately",
      "Systemic corticosteroids are reserved for scleritis, and even there treatment usually begins with non-steroidal anti-inflammatories."],
 ...
 dict(topic="Scleritis", io=IOA, lead="next step", slot="referral",
   q="A 52-year-old woman with rheumatoid arthritis has severe deep boring eye pain that wakes her at night and radiates to her cheek. The sclera has a bluish-purple hue (violaceous discoloration, the choroid showing through thinned sclera). Which is the most appropriate next step?",
   opts=[
     ["Urgent referral to ophthalmology",
 ...
 dict(topic="Scleritis", io=IOA, lead="education", slot="prognosis",
   ...
   opts=[
     ["This is the expected first sign of response — decreased pain comes before any change in appearance",
      "Correct — pain settles before the redness does, so a still-inflamed eye with less pain indicates the treatment is working."],
     ["Treatment has failed and should be changed immediately",
      "Changing treatment now would discard a regimen that is working; appearance lags behind pain by days."],
 ...
 dict(topic="Corneal ulcer", io=IOA, lead="next step", slot="referral",
   ...
   opts=[
     ["Emergent referral to ophthalmology",
      "Correct — an ulcer is an established break in the cornea that can perforate, so it goes above the 24-hour urgency of keratitis to emergent referral."],
 ...
 dict(topic="Pre-septal cellulitis", io=IOB, lead="next step", slot="escalation",
   ...
   opts=[
     ["Hospital admission for broad-spectrum intravenous antibiotics",
      "Correct — the findings place the infection in front of the orbital septum, but a child of five or under is admitted because spread behind it is faster and harder to detect."],
 ...
 dict(topic="Orbital cellulitis", io=IOA, lead="complication", slot="complication",
   ...
   opts=[
     ["Intracranial spread causing meningitis or cavernous sinus thrombosis",
      "Correct — the orbital veins are valveless and drain backwards into the cavernous sinus, so infection can track intracranially."],
 ...
 dict(topic="Dacryocystitis", io=IOA, lead="education", slot="prognosis",
   ...
   opts=[
     ...
     ["He will need lifelong daily antibiotics",
      "Indefinite antibiotics do not relieve the nasolacrimal obstruction that caused the infection; the drainage system still has to be assessed."],
     ["He will need annual computed tomography scanning",
      "Serial scanning adds nothing; the follow-up question is whether the duct drains, which probing and irrigation answer."]],
   c=0, cite=c(25)),
 ...
 dict(topic="Dacryoadenitis", io=IOA, lead="avoid", slot="avoid",
   ...
   opts=[
     ...
     ["Giving paracetamol for pain",
      "Analgesia is appropriate and does not obscure the diagnosis; it is immunosuppression before excluding infection that is dangerous."],
     ["Reassessing after 24 hours of empiric oral antibiotics",
      "Empiric antibiotics with review at 24 hours is a reasonable approach when the cause is unclear, precisely because it treats infection while the picture declares itself."],
     ["Monitoring for signs of orbital involvement",
      "Watching for proptosis and restricted movement is essential, since those signal spread behind the orbital septum."]],
   c=0, cite=cn(23)),
 ...
 dict(topic="Chalazion", io=IOA, lead="next step", slot="referral",
   ...
   opts=[
     ...
     ["Reassure her that recurrence is normal and no action is needed",
      "Recurrence in the same spot is exactly what raises concern, because sebaceous carcinoma of the meibomian gland masquerades as a recurrent chalazion."]],
   c=0, cite=c(21)),
 ...
 dict(topic="Hordeolum", io=IOA, lead="next step", slot="escalation",
   ...
   opts=[
     ["Refer to ophthalmology for incision and drainage",
      "Correct — a hordeolum that has not improved after about two weeks of conservative treatment is referred for incision and drainage."],
     ...
     ["Perform incision and drainage in the clinic yourself",
      "Incision near the lid margin risks damaging the margin and the tear drainage apparatus, so the procedure is done by ophthalmology."]],
   c=0, cite=c(21)),
 ...
 dict(topic="Blepharitis", io=IOA, lead="first-line", slot="first-line",
   ...
   opts=[
     ["Lid hygiene", "Correct — warm compresses with lid scrubs clear the obstructed meibomian glands and crusted margin, which is the underlying problem."],
 ...
 dict(topic="Blepharitis", io=IOA, lead="education", slot="prognosis",
   ...
   opts=[
     ["It is chronic and can be controlled rather than cured, though symptoms often improve substantially over weeks",
      "Correct — the lid margin disease persists, so treatment controls it rather than eradicating it, and stopping hygiene brings symptoms back."],
