#!/usr/bin/env python3
"""Partition a CMS I Exam 5 four-option pool into two 30s.

    python3 cms_e5_partition.py l27io | l27vig   # Lecture 27 Arterial Occlusive Disease and Aortic Aneurysm
    python3 cms_e5_partition.py l28io | l28vig   # Lecture 28 Cardiomyopathy

Copied from the GUARDED path of cms_e4_partition.py (2026-09-24), which is the
Carter build for Exam 4; the Hypertension LEGACY path is not carried over. The
scoring, rotation and every guard are unchanged:

      schema     four options, key authored first and opening "Correct", every
                 option explained (distractors >= 60 chars), a slide citation on
                 every question
      slot       every question tagged with one of the 16 fact slots
      io         every question carries one of the lecture's VERBATIM syllabus
                 objectives (scope module IOS); Set 1 is scored on covering all
      scope      no question cites an excluded slide; no dosing; no brand names;
                 no mechanism-of-action stems; the lecture's own banned patterns
      acronyms   no bare acronym in a stem, option or explanation -- written
                 out, or ABBREV (full term); Carter: "no acronyms in questions"
      spelling   US spelling (2026-09-23); "Grey Turner" exempt as a proper noun
      ...
      vignette   Set 2 only: every stem opens on a patient; named findings carry
                 their description in parentheses (scope module NAMED); explicit
                 lead= from the vocabulary; a stem asking for the diagnosis must
                 be labeled diagnosis
      dx cap     at most 6 diagnosis lead-ins per 30, BY CONSTRUCTION
      skew       no lead-in type above 40% of a set (asserted)

Exam 5 differences: module prefix cms_e5l<N>_, the Exam 5 decks' brand names
added to the brand guard (Plavix, Pletal, Trental, Activase, Cordarone,
Norpace, and the usual anticoagulant and calcium channel blocker brands), and
"Grey Turner" exempted from the UK-spelling guard.
"""
...
WHICH = sys.argv[1] if len(sys.argv) > 1 else "l27io"
GUARDED = {
 # key: (lecture tag, is vignette set, seed)
 "l27io": ("l27", False, 20260925 + 271),
...
# ============================================================================
# GUARDED PATH -- Carter's lectures 27 and 28 (the only path in this driver).
# ============================================================================
...
# A stem that asks for the diagnosis must be LABELED diagnosis, whatever else
...
# Exam 5 adds ST (the ST segment) and the augmented limb leads: conventional ECG
# labels like QRS, not abbreviations of a phrase (no_abbreviations_content_policy).
_ACRO_OK = re.compile(r"^(?:I{1,3}|IV|VI{0,3}|IX|X|S[1-4]|mmHg|QRS|ST|aV[FLR])$")
...
_UK = re.compile(r"\b\w*(?:aemi|oedem|oesoph|oestr|paediat|orthopaed|ischaem|haemo(?!philus)|haema|haemorrh|"
                 ...
                 r"counsell\w*|labell\w*|modell\w*|travell\w*|ageing|programme|manoeuvres?|grey(?! turner)|"
...
# Exam 5 (2026-09-25): the Exam 4 pattern ended each -is stem with \w*, which
# flagged correct US words -- "specialist", "characteristic", "organism". The
# stems now only match the -ise/-ised/-ising/-isation endings.
...
def load_guarded(lec, vig):
    scope = importlib.import_module("cms_e5%s_scope" % lec)
    pat = "cms_e5%s_%s_*.py" % (lec, "vig" if vig else "pool")
...
def guarded_main():
    ...
    OUT_JSON = "cms_e5%s_%s.json" % (lec, "vig_sets" if vig else "sets")
