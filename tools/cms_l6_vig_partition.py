#!/usr/bin/env python3
"""Partition CMS I Lecture 6 Set 2 (Cutaneous Viral and Fungal Infections vignettes) into two 30s.

CMS format per [[cms_exam_spec]]: Set 1 is 2 x 30 on the instructional
objectives, Set 2 is a separate 2 x 30 of vignettes. This builds Set 2.

THE SLIDE IS AUTHORITATIVE (Jaxon, 2026-08-20). Every stem is grounded in the
deck; no lecture audio content is admitted without checking it against a slide.
...
LENGTH BIAS. Raw rates by pool: A 9%, B 18%, C 19%. Lower than Lecture 7's Set 2
because the distractors were written to the answer's shape from the start
instead of being padded afterwards. All three finish at 0%.
"""
import sys, os, json, random, re
from collections import Counter
HERE = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, HERE)
from cms_l6_vig_a import VIG_A
from cms_l6_vig_b import VIG_B
from cms_l6_vig_c import VIG_C

POOL = VIG_A + VIG_B + VIG_C
...
_PT = re.compile(r"\b\d+[- ]year[- ]old|\b\d+[- ]month[- ]old|\b\d+[- ]week[- ]old|"
                 ...
                 r"the patient|a patient|an? (immunocompromised|pregnant|susceptible) patient|"
                 r"\ba (child|man|woman|mother|grandfather|colleague)\b|the parents|"
                 r"a school nurse|several patients|his |her ", re.I)
...
if __name__ == "__main__":
    ...
    json.dump({"set1": s1, "set2": s2}, open(os.path.join(HERE, "cms_l6_set2.json"), "w"),
              ensure_ascii=False, indent=1)
    print("wrote cms_l6_set2.json")
