...
POOL_A = [
 dict(topic="Fungal biology", io=IO2,
   ...
   opts=[
     ["They are eukaryotic, with a rigid cell wall containing chitin, a cell membrane built on ergosterol, different ribosomes and a distinct nuclear membrane",
      "Correct. Antibacterials target prokaryotic structures fungi do not possess, and because fungal cells are eukaryotic like ours, finding a safe target is harder than with bacteria."],
 ...
 dict(topic="Fungal biology", io=IO9,
   ...
   opts=[
     ...
     ["Advanced age, malnutrition, smoking, alcohol use, and occupational exposure",
      "These are general risk factors for ill health; the specific predispositions are a breached barrier, impaired immunity, metabolic derangement, suppressed competing flora and a warm moist environment."],
 ...
 dict(topic="Fungal biology", io=IO9,
   ...
   opts=[
     ...
     ["Acute, subacute and chronic, according to the duration of infection",
      "Duration does not determine treatment here; what matters is how deep the infection sits, since that dictates whether topical or systemic therapy can reach it."],
     ...
     ["Endemic, sporadic and epidemic, according to geographic distribution",
      "Geography influences which organism is likely, but the classification used is anatomical depth, from superficial keratin to internal organs."]],
   c=0, cite=c(92)),
 ...
 dict(topic="Mechanisms", io=IO2,
   ...
   opts=[
     ["Griseofulvin",
      "Correct. Griseofulvin disrupts the mitotic spindle and concentrates in newly formed keratin, which is why treatment must continue until the infected nail or hair grows out."],
     ["Flucytosine",
 ...
 dict(topic="Flucytosine", io=IO9,
   ...
   opts=[
     ...
     ["It is actively pumped into fungal cells by a transporter absent from human cells",
      "No such transporter is invoked; selectivity comes from cytosine deaminase, which fungi have and human cells do not, so the drug is only activated inside the fungus."]],
   c=0, cite=c(100)),
 ...
 dict(topic="Anthelmintics", io=IO9,
   ...
   opts=[
     ["They inhibit formation of helminth microtubules and block glucose uptake, leading to parasite death",
      "Correct. Blocking microtubule formation stops the helminth taking up glucose, so it is starved rather than killed outright, which is why treatment can take days."],
