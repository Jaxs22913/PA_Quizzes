#!/usr/bin/env python3
"""Partition CMS I Lecture 7 Set 2 (Benign Skin Lesions vignettes) into two 30s.
...
the worst, as it was in Set 1, because comparing seven vascular lesions makes
the correct answer compound by default. All three were repaired by TRIMMING THE
CORRECT ANSWER and moving the detail into its explanation, which is one edit per
question instead of three. The pool finishes at 0%.
...
from cms_l7_vig_a import VIG_A
from cms_l7_vig_b import VIG_B
from cms_l7_vig_c import VIG_C
...
_PT = re.compile(r"\b\d+[- ]year[- ]old|\b\d+[- ]month[- ]old|\b\d+[- ]week[- ]old|"
                 ...
                 r"the patient|a patient|four patients|the parents|a ward sister|"
                 r"his |her ", re.I)
...
if __name__ == "__main__":
    ...
    json.dump({"set1": s1, "set2": s2}, open(os.path.join(HERE, "cms_l7_set2.json"), "w"),
              ensure_ascii=False, indent=1)
    print("wrote cms_l7_set2.json")
