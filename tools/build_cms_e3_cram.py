...
topics = [
 dict(id="tests", label="The tests", color="#6a4fa3", tag="objectives b to f", rows=[
  ...
  ["CONDUCTIVE", "Weber lateralizes <b>TO the BAD ear</b>. Rinne: <b>BC &ge; AC</b>. Voice stays SOFT. Hearing BETTER in noise."],
  ["SENSORINEURAL", "Weber lateralizes <b>AWAY, to the GOOD ear</b>. Rinne: <b>AC &gt; BC &mdash; SAME AS NORMAL</b>. Voice LOUD. Hearing WORSE in noise."],
  ...
  ["AUDIOMETRY &mdash; ALL BY 20s", "Normal 0&ndash;20 &middot; MILD 20&ndash;40 &middot; MODERATE 40&ndash;60 &middot; SEVERE 60&ndash;80 &middot; <b>PROFOUND &gt;80 dB</b>. The prevalence percentages are explicitly NOT to be memorized."],
 ...
 dict(id="om", label="The otitis media family", color="#8f5aa8", rows=[
  ...
  ["ITS CAUSE &mdash; the slide is incomplete", "<b>FAR AND AWAY MOST ARE VIRAL.</b> Bacterial three: <b>S. PNEUMONIAE, H. INFLUENZAE, M. CATARRHALIS</b>. M. catarrhalis has OVERTAKEN H. influenzae because of VACCINATION &mdash; ask IMMUNIZATION STATUS."],
 ...
 dict(id="canal", label="The canal", color="#a4502a", rows=[
  ...
  ["CARCINOMA OF THE CANAL", "<b>OTITIS EXTERNA THAT WILL NOT RESPOND</b> + BLOODY otorrhea + FRIABLE canal. Often MISDIAGNOSED. <b>BIOPSY.</b> EMERGENT."],
 ]),
 dict(id="drum", label="The drum, pressure and fixation", color="#7a5a2e", rows=[
  ...
  ["CHOLESTEATOMA", "<b>KERATIN in a RETRACTION POCKET</b> + <b>RECURRENT OTORRHEA WITH NO OTITIS EXTERNA</b>. Not a neoplasm, no cholesterol. CT for extent. <b>SURGERY</b> &mdash; it erodes bone."],
  ...
  ["AURICULAR HEMATOMA", "Sub-perichondrial. Landmarks LOST. <b>DRAIN WITHIN 7 DAYS</b> &mdash; then splint. Re-check at 12&ndash;24h because it can appear LATE. Otherwise CAULIFLOWER EAR."],
 ]),
 dict(id="snhl", label="Sensorineural causes worth knowing cold", color="#2a5f8f", rows=[
  ...
  ["GLOMUS TUMOR", "<b>PULSATILE TINNITUS</b> + vascular middle ear mass. Can paralyze CN IX, X, XI."],
 ...
 dict(id="nose", label="Nose and sinuses", color="#9c5230", tag="Lecture 17", rows=[
  ["ACUTE SINUSITIS", "<b>UNDER 4 WEEKS.</b> <b>90&ndash;98% VIRAL</b>; only 0.5&ndash;2% superinfect. Pain <b>WORSE BENDING FORWARD</b>, localizes to the sinus. 1 in 8 adults; <b>5th leading reason antibiotics are prescribed</b>."],
  ...
  ["DISCHARGE COLOR", "<b>Yellow/green = LEAST helpful.</b> Clear = viral or allergic. Yellow AND putrid = bacterial. <b>BLACK = FUNGUS.</b> Rust = possible S. pneumoniae."],
  ...
  ["PERFORATED SEPTUM", "<b>INTRANASAL STEROIDS or COCAINE</b> &mdash; chronic ischemia. Rarely GRANULOMATOSIS WITH POLYANGIITIS or secondary SYPHILIS. Treat the cause; else septoplasty."],
  ["SEPTAL HEMATOMA", "Between septum and PERICHONDRIUM. Usually TRAUMA; associated with nasal fracture. <b>DRAIN via intranasal incision under general anesthesia.</b>"],
  ...
  ["EPISTAXIS &mdash; POSTERIOR", "<b>SPHENOPALATINE ARTERY.</b> Significant hemorrhage. <b>HIGHER RISK because of ASPIRATION</b> and subsequent infection."],
  ...
  ["EPISTAXIS WORKUP", "<b>PT/INR is NOT routine</b> &mdash; only if anticoagulated. Hematocrit + type and crossmatch if massive, with <b>2 large-bore IV lines</b>."],
  ["NASAL FOREIGN BODY", "<b>UNILATERAL PURULENT FOUL-SMELLING discharge in a young child.</b> Floor of the passage under the INFERIOR turbinate, or in front of the MIDDLE turbinate. Visualization is the diagnosis."],
  ["NASAL FRACTURE &mdash; no x-ray if", "<b>ALL FOUR:</b> tenderness/swelling ISOLATED to the bony bridge &middot; breathes through EACH naris &middot; nose STRAIGHT, no septal deviation &middot; <b>NO SEPTAL HEMATOMA</b>. Otherwise film it. Initial care: ICE + head of bed up."],
  ["NASAL POLYPS", "<b>GRAY, GLISTENING</b> masses &rarr; congestion, thick discharge, <b>ANOSMIA</b>. Asthma 20&ndash;50%, <b>allergic fungal sinusitis 85%</b>, aspirin intolerance 8&ndash;20%, alcohol intolerance 50%, Churg-Strauss 50%, <b>CF 5&ndash;44%</b>. <b>EVALUATE EVERY CHILD WITH MULTIPLE POLYPS FOR CF AND ASTHMA</b> &mdash; chloride sweat test. Surgery gives only TEMPORARY relief."],
  ["ALLERGIC RHINITIS", "<b>CLEAR discharge from BOTH nostrils</b>, <b>BLUISH hue</b>, edematous mucosa, &plusmn; polyps. <b>80% end up on 2+ medicines.</b> Antihistamine + leukotriene inhibitor at night + intranasal steroid; ipratropium; immunotherapy. <b>Allergy creates the environment for infection but is not itself an -itis.</b>"],
 ...
 dict(id="neck", label="Neck masses", color="#2f6b6f", tag="Lecture 18", rows=[
  ...
  ["THE TWO CONGENITAL SIGNS", "<b>THYROGLOSSAL DUCT CYST moves VERTICALLY with SWALLOWING or TONGUE PROTRUSION</b> &mdash; pathognomonic, and why the SISTRUNK operation takes the CENTER OF THE HYOID. <b>LYMPHANGIOMA TRANSILLUMINATES.</b>"],
  ...
  ["THE EXPOSURE HISTORIES", "<b>CATS scratch</b> &rarr; <i>Bartonella henselae</i> &middot; <b>CAT FECES / undercooked meat</b> &rarr; toxoplasmosis &middot; <b>RABBITS, TICKS</b> &rarr; tularemia (STREPTOMYCIN) &middot; <b>UNPASTEURIZED MILK</b> &rarr; brucellosis (total-body nodes)."],
  ...
  ["THE WORKUP", "<b>FNA BIOPSY = STANDARD OF CARE</b>, minimum <b>4 PASSES</b>; separates neoplasm from inflammation and carcinoma from lymphoma. <b>FNA not EXCISIONAL</b> &mdash; excision spills tumor. <b>AVOID IODINE CONTRAST if thyroid cancer is suspected.</b>"],
 ...
 dict(id="oral", label="Mouth, throat and jaw", color="#8c4a5f", tag="Lecture 19", rows=[
  ...
  ["APHTHOUS vs HERPES", "<b>APHTHOUS &rarr; NON-KERATINIZED, freely moving mucosa</b> (buccal, labial, non-attached gingiva); yellow-gray center, RED HALO. <b>HERPES &rarr; keratinized surfaces</b>, with a <b>24-hour BURNING PRODROME</b>. Recurrent aphthous = <b>SUTTON DISEASE</b>."],
  ...
  ["MONONUCLEOSIS", "<b>Fever + tonsillar pharyngitis + cervical adenopathy</b>, ages 15&ndash;24, <b>EBV 90%</b>. <b>MONOSPOT falsely negative in WEEK 1</b> (heterophile antibodies take a week to appear) &mdash; <b>slide 84 prints &ldquo;falsely positive&rdquo;, which is wrong; Prof. Shah confirmed so in class on 9 September.</b> <b>NO CONTACT SPORTS ~1 MONTH</b> &mdash; splenic rupture. <b>GIVING PENICILLIN CAUSES A RASH</b> &mdash; that rash is often how the diagnosis gets made."],
  ...
  ["DIPHTHERIA", "<b>TENACIOUS GRAY MEMBRANE</b> over tonsils and pharynx, unimmunized child. <b>ANTITOXIN from the CDC</b> + PCN or erythromycin <b>14 days</b>. <b>MYOCARDITIS</b> and <b>cranial neuropathy</b> are the killers."],
