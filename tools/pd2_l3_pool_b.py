...
POOL_B = [
 dict(topic="Proptosis", io=IO, slot="initial test",
   ...
   opts=[
     ["Stand behind the seated patient and inspect from above",
      "Correct — the technique Beck demonstrated and kept in scope. Drawing the lid slightly upward to compare the corneas against the lower lids."],
     ["Sit facing the patient and compare the two eyes directly",
      "The material specifically asks you to look down from above."],
 ...
 dict(topic="Proptosis", io=IO, slot="etiology",
   ...
   opts=[
     ["Retrobulbar haemorrhage, orbital cellulitis, orbital tumour, and Graves disease",
      "Correct — the four orbital causes."],
 ...
 dict(topic="Eyebrows", io=IO, slot="test finding",
   ...
   opts=[
     ["Hypothyroidism", "Correct — the association for lateral eyebrow loss."],
     ["Hyperthyroidism", "The material associates lid lag with hyperthyroidism, not eyebrow loss."],
     ["Seborrhoeic dermatitis", "That causes scaliness of the eyebrows."],
     ["Iron deficiency", "Not an association it is."]],
   c=0, cite=c(27)),

 dict(topic="Eyebrows", io=IO, slot="test finding",
   q="Scaliness of the eyebrows suggests what?",
   opts=[
     ["Seborrhoeic dermatitis", "Correct — the association for eyebrow scaling."],
     ["Hypothyroidism", "That causes lateral sparseness instead."],
     ["Contact dermatitis", "Not the named association."],
     ["Psoriasis", "Not the named association here."]],
   c=0, cite=c(27)),

 dict(topic="Ptosis", io=IO, slot="etiology",
   q="Besides senile and congenital forms, which three causes of ptosis are there?",
   opts=[
     ["Myasthenia gravis, oculomotor nerve damage",
      "Correct — the three, spanning neuromuscular junction, motor nerve and sympathetic chain. Damage to the sympathetic supply (Horner syndrome)."],
     ...
     ["Thyroid eye disease, orbital cellulitis and tumour",
      "Those cause proptosis, which is a different finding."],
     ["Blepharitis, chalazion and dermatochalasis",
      "Those are lid conditions but not the ptosis causes."]],
   c=0, cite=c(29)),
 ...
 dict(topic="Ptosis", io=IO, slot="etiology",
   ...
   opts=[
     ["Weakened muscle, relaxed tissues",
      "Correct — the three-part mechanical explanation. The weight of herniated fat."],
 ...
 dict(topic="Hordeolum and chalazion", io=IO, slot="differential",
   ...
   opts=[
     ["The hordeolum is painful and sits AT THE LID'S EDGE",
      "Correct — it is both the pain and the position. The chalazion is a chronic, non-painful meibomian lesion that generally is not at the margin."],
 ...
 dict(topic="Chalazion", io=IO, slot="test finding",
   ...
   opts=[
     ...
     ["At the lateral canthus",
      "Not where the material places it."]],
   c=0, cite=c(33)),
 ...
 dict(topic="Blepharitis", io=IO, slot="etiology",
   ...
   opts=[
     ["Bacterial infection and atopic dermatitis",
      "Correct — the two associations to know."],
 ...
 dict(topic="Xanthelasma", io=IO, slot="test finding",
   ...
   opts=[
     ["Along the NASAL portions of one or both eyelids — consider lipid disorders",
      "Correct — it is both the position and the inference."],
     ["Along the temporal portions of the lids — consider thyroid disease",
      "It is the nasal portion and lipid disorders."],
 ...
 dict(topic="Lid position", io=IO, slot="differential",
   ...
   opts=[
     ["Posteriorly misdirected eyelashes", "Correct — the definition, listed alongside entropion and ectropion."],
 ...
 dict(topic="Nasolacrimal duct", io=IO, slot="initial test",
   ...
   opts=[
     ["Ask the patient to look up, press on the lower lid near the medial canthus just inside the bony orbit to compress the lacrimal sac, and look for fluid regurgitating from the puncta — mucopurulent fluid suggests obstruction",
      "Correct — the full technique and its positive finding."],
     ["Instil fluorescein and time its disappearance from the tear film",
      "A real test, but not the one it is here."],
 ...
 dict(topic="Nasolacrimal duct", io=IO, slot="avoid",
   ...
   opts=[
     ["If the area is significantly inflamed or tender",
      "Correct — it is to avoid it in that situation."],
     ["If the patient wears contact lenses",
      "Not a contraindication it is."],
 ...
 dict(topic="Upper lid eversion", io=IO, slot="initial test",
   ...
   opts=[
     ["Ask the patient to look down and relax",
      "Correct — the step-by-step, including the 1 cm landmark. Raise the lid slightly so the lashes protrude, grasp the lashes and pull down and forward, then place a stick at least 1 cm above the lid margin at the upper border of the tarsal plate and push down as you raise the lid edge."],
     ["Ask the patient to look up, then pull the lid upward directly",
      "Looking up defeats the eversion; it is downgaze."],
     ["Press on the globe itself to push the lid outward",
      "It is explicitly not to press on the eyeball."],
     ["Use a cotton bud placed at the lid margin itself",
      "The material places the stick at least 1 cm above the margin."]],
   c=0, cite=c(42)),

 dict(topic="Upper lid eversion", io=IO, slot="avoid",
   q="When must the lid never be everted?",
   opts=[
     ["If rupture of the globe is suspected", "Correct — the material states this as an absolute."],
 ...
 dict(topic="Subconjunctival haemorrhage", io=IO, slot="test finding",
   ...
   opts=[
     ["Pain absent, vision and pupil unaffected, no discharge, cornea clear",
      "Correct — the four negatives, which are what make it benign."],
 ...
 dict(topic="Subconjunctival haemorrhage", io=IO, slot="escalation",
   ...
   opts=[
     ["In trauma, and when the haemorrhage encircles the entire cornea",
      "Correct — it is both the mechanism and the 360-degree pattern."],
     ...
     ["When both eyes are affected",
      "Bilaterality is not the warning sign."]],
   c=0, cite=c(44)),
 ...
 dict(topic="Conjunctivitis", io=IO, slot="test finding",
   ...
   opts=[
     ["Diffuse dilation of the conjunctival vessels",
      "Correct — peripheral maximum is what separates it from a ciliary pattern. With redness maximal PERIPHERALLY."],
 ...
 dict(topic="Conjunctivitis", io=IO, slot="test finding",
   q="What happens to vision and the pupil in conjunctivitis?",
   opts=[
     ["Vision is not affected except for mild blurring from discharge",
      "Correct — preserved vision and a normal pupil are what make it benign. The pupil is not affected."],
 ...
 dict(topic="Injection pattern", io=IO, slot="differential",
   ...
   opts=[
     ["Keratitis, iritis, or acute glaucoma",
      "Correct — the three, and this is the pattern that must not be dismissed as conjunctivitis."],
 ...
 dict(topic="Nodular episcleritis", io=IO, slot="etiology",
   ...
   opts=[
     ["Rheumatoid arthritis and lupus erythematosus",
      "Correct — the two associations."],
     ["Diabetes and hypertension",
      "Those drive retinopathy rather than this finding."],
     ["Graves disease and myasthenia gravis",
 ...
 dict(topic="Red eye chart", io=IO, slot="test finding", chart=True,
   ...
   opts=[
     ["Pain, decreased vision, unequal pupils",
      "Correct — the chart's four backup clues when the injection pattern is unhelpful. A less than perfectly clear cornea."],
 ...
 dict(topic="Red eye chart", io=IO, slot="education", chart=True,
   ...
   opts=[
     ["That she genuinely thinks students should be very familiar with it, because it helps compare and contrast the common important eye conditions",
      "Correct, and it is one of the clearest points in this topic."],
