...
{"id": "bias", "label": "Bias, Chance & Validity", "color": "#7a2d47", "rows": [
 ...
 ["Measurement bias", "The method of measurement gives incorrect results &mdash; blood pressure taken without a standardised procedure every time."],
 ...
 ["Procedural bias", "Interview/questionnaire results affected by method of delivery &mdash; a post-op questionnaire while anaesthesia is wearing off."],
 ...
 ["Reporting vs publication bias", "<b>Reporting</b> = only a subset of the data is made available (the study appears, not all its findings). <b>Publication</b> = non-significant research is less likely to be published, via editor preference and sponsors avoiding unfavourable results."],
...
{"id": "evidence", "label": "Evidence & the Hierarchy", "color": "#8a5a2b", "rows": [
 ["Origins", "<b>McMaster University, 1970s.</b> <b>Sackett</b> gave the core concepts and definition; <b>Cochrane</b> advocated randomised trials and systematic reviews to verify treatment effectiveness."],
 ...
 ["Higher levels are&hellip;", "Least vulnerable to bias &middot; more generalisable &middot; outcomes more likely attributable to the construct studied."],
 ["Pyramid, base to apex", "Expert opinion/background &rarr; case-control, case series/reports &rarr; cohort &rarr; randomised controlled trials &rarr; <span style='white-space:nowrap'>critically-appraised articles</span> &rarr; critically-appraised topics &rarr; <b>systematic reviews</b>. (The laboratory version adds in vitro and animal research below case reports.)"],
 ["Filtered vs unfiltered", "<b>Filtered</b> = already appraised for you (systematic reviews, critically-appraised topics and articles). <b>Unfiltered</b> = primary studies you appraise yourself (randomised trials, cohort, case-control)."],
...
{"id": "design", "label": "Study Design", "color": "#2f5d6b", "rows": [
 ...
 ["Study design", "The organisation and plan for data collection and analysis. <b>Strong design &rarr; strong data &rarr; valid, reliable results.</b>"],
 ["Descriptive vs analytic", "<b>Descriptive</b> describes one group (case report, case series). <b>Analytic</b> compares two or more groups to draw inference for a population (randomised trials, cohort, case-control)."],
 ["Experimental vs observational", "<b>Experimental</b>: the investigator <i>assigns and manipulates</i> the intervention (randomised trial). <b>Observational</b>: the exposure is <i>observed</i> (cohort, case-control). Cross-sectional is observational and appears under both descriptive and analytic."],
 ...
 ["Design goal", "An <b>unbiased comparison</b> between a group with the factor and one without. Good design permits group comparison, quantification of risk difference, <b>determination of temporal sequence</b>, and minimised bias."],
 ...
 ["Randomised controlled trial", "Experimental; tests hypotheses. Clear hypothesis, protocol, results applicable to a larger population, <b>random assignment</b>, single or double blinding, adequate sample size, accounts for withdrawals and drop-outs."],
 ...
 ["Typical sequence", "Clinical observations &rarr; available data &rarr; case-control &rarr; cohort &rarr; randomised trials."],
]},
...
{"id": "rates", "label": "Rates, Incidence & Prevalence", "color": "#1f5c3a", "rows": [
 ...
 ["Prevalence clinically", "Prioritises the differential (<i>horses, not zebras</i>) and sets the <b>pretest probability</b>."],
 ...
 ["Distribution: person, place, time", "<b>Person</b> = profile of those affected, demographics <i>and</i> behaviour. <b>Place</b> = <b>endemic</b>, limited by geography. <b>Time</b> = <b>epidemic</b> (new cases concentrated in time) and <b>pandemic</b> (widespread; rapid like 1900s influenza or slow like HIV/AIDS)."],
 ["Surveillance", "Active or passive; national, state, county. Establishes a <b>baseline</b>. Functions: magnitude &middot; natural history &middot; detect outbreaks &middot; document geographic spread &middot; test aetiology (via case-control or cohort) &middot; evaluate control &middot; monitor agent change. Strategies: screen high-risk groups &middot; symptoms/syndrome &middot; <b>mortality</b> (some cases detected only at death) &middot; provider reporting &middot; laboratory reporting."],
 ["Outbreak investigation", "Epidemic vs endemic &rarr; attack rate &rarr; establish diagnosis &rarr; <b>case definition</b> &rarr; confirm an epidemic truly exists &rarr; characterise by time, place, person &rarr; <b>epi curve</b> (cases vs time) &rarr; hypotheses about source/type/route &rarr; test &rarr; control measures &rarr; solve source &rarr; <b>decide it is over</b>. (Philadelphia Legionnaires', 1976.)"],
]},

{"id": "data", "label": "Data, Validity & Variation", "color": "#4a3a7a", "rows": [
 ["Three types of data", "<b>Nominal</b> &mdash; categories, no order (blood group, eye colour); <b>dichotomous</b> is nominal split in two. <b>Ordinal</b> &mdash; order, <i>no defined intervals</i> (cancer stage I&ndash;IV, oedema 1+&ndash;4+); you can say more/less, not by how much. <b>Interval</b> &mdash; order <i>with</i> equal defined intervals; <b>continuous</b> (blood pressure, weight, limited by measurement precision) or <b>discrete</b> (number of children)."],
 ...
 ["Defining abnormality", "Use values associated with <b>disease, disability or death</b>; a clinically recognisable difference from healthy. <b>Repeat abnormal tests</b> &mdash; regression to the mean. Cutoffs are <b>often arbitrary</b>. <b>Be careful of labelling.</b>"],
]},
...
{"id": "tests", "label": "Diagnostic Tests", "color": "#a3341f", "rows": [
 ...
 ["Gold standard", "A procedure believed to identify the disease <b>with certainty</b> &mdash; culture for strep, biopsy for cancer, catheterisation for coronary disease, endoscopy for peptic ulcer. Often expensive, invasive/risky, unacceptable, or not easily completed, so alternatives are usually the initial choice."],
