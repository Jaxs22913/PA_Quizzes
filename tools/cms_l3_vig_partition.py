#!/usr/bin/env python3
"""Partition CMS I Lecture 3 SET 2 (the vignettes) into two 30s.
...
invisible in the finished quiz:
  ...
  management. The guard fails the build if any one lead-in type exceeds 40% of
  a set.
...
from cms_l3_vig_lengthfix import SLOT_FIXES
...
LEAD = {
 "diagnosis": r"most likely diagnosis|most likely explanation|most likely underlying diagnosis|which subtype|most likely mechanism|which phenomenon",
...
if __name__ == "__main__":
    ...
    json.dump({"set1": s1, "set2": s2}, open(os.path.join(HERE, "cms_l3_set2.json"), "w"),
              ensure_ascii=False, indent=1)
    print("wrote cms_l3_set2.json")
