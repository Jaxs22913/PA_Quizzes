 ...
 dict(topic="Squamous cell carcinoma", io=IO, slot="initial test",
   ...
   opts=[
     ...
     ["Shave biopsy only, since deeper sampling risks seeding the tumour",
      "Squamous cell carcinoma does not seed along a biopsy tract, and a shave alone may not reach deep enough to show whether the tumour has invaded."],
 ...
 dict(topic="Squamous cell carcinoma", io=IO, slot="initial test",
   ...
   opts=[
     ...
     ["Sentinel lymph node biopsy in every case over one centimetre",
      "Sentinel node biopsy belongs to melanoma staging; in squamous cell carcinoma the nodes are palpated, with imaging reserved for high-risk tumours."],
 ...
 dict(topic="Basal cell carcinoma", io=IO, slot="initial test",
   ...
   opts=[
     ...
     ["It is a clinical diagnosis and biopsy is not usually required",
      "Sampling is needed, because only the histologic subtype distinguishes a superficial tumour from an infiltrative one that extends well beyond its visible edge."]],
   c=0, cite=c(36)),
 ...
 dict(topic="Basal cell carcinoma", io=IO, slot="avoid",
   ...
   opts=[
     ...
     ["Routine imaging of the affected region is required before any excision",
      "Basal cell carcinoma almost never metastasises, so imaging before excising a typical localised lesion adds cost without changing the operation."],
 ...
 dict(topic="Kaposi sarcoma", io=IO, slot="initial test",
   ...
   opts=[
     ...
     ["Clinical appearance alone, since the violaceous colour is characteristic",
      "Colour is unreliable across skin tones and overlaps with bacillary angiomatosis and purpura, which is why biopsy rather than appearance confirms it."],
 ...
 dict(topic="Kaposi sarcoma", io=IO, slot="differential",
   ...
   opts=[
     ...
     ["Colour, size and symmetry of the lesions",
      "Colour, size and symmetry cannot separate Kaposi sarcoma from bacillary angiomatosis or purpura; immune context, distribution and biopsy can."],
 ...
 dict(topic="Kaposi sarcoma", io=IO, slot="initial test",
   ...
   opts=[
     ...
     ["Test for human immunodeficiency virus only if the patient reports risk factors",
      "Reported risk factors miss many infections, so testing is offered whenever status is unknown rather than only on disclosure."],
 ...
 dict(topic="Kaposi sarcoma", io=IO, slot="escalation",
   ...
   opts=[
     ...
     ["Bronchoscopy and endoscopy in every newly diagnosed patient",
      "Scoping every patient subjects asymptomatic people to invasive procedures whose findings would not change management."],
     ["Computed tomography of the chest in every newly diagnosed patient",
      "A chest radiograph is the screening step where pulmonary involvement is possible; computed tomography of every patient is not required."],
 ...
 dict(topic="Cutaneous T-cell lymphoma", io=IO, slot="initial test",
   ...
   opts=[
     ...
     ["The oldest and most heavily treated lesion, since it is most established",
      "Topical steroids and phototherapy suppress the atypical lymphocytic infiltrate, so a treated lesion can look benign under the microscope."],
     ["Any lesion, since the histology is uniform across the disease",
      "Lesions differ in stage and activity, and an inactive or resolving patch may show nothing diagnostic."],
 ...
 dict(topic="Cutaneous T-cell lymphoma", io=IO, slot="avoid",
   ...
   opts=[
     ...
     ["The diagnosis is excluded and an alternative should be pursued",
      "Early disease is histologically subtle, so a nondiagnostic result is common and does not overturn a clinical picture that fits."],
 ...
 dict(topic="Cutaneous T-cell lymphoma", io=IO, slot="escalation",
   ...
   opts=[
     ...
     ["Complete blood count alone, repeated at every visit",
      "A blood count alone cannot detect circulating malignant cells; advanced disease needs Sezary cell assessment, gene rearrangement studies and flow cytometry."],
 ...
 dict(topic="Actinic keratosis", io=IO, slot="test finding",
   ...
   opts=[
     ...
     ["Hypertrophic versus atrophic actinic keratosis, because that guides field therapy",
      "The distinction is not a two-way split; the pathologist separates actinic keratosis from in situ and from invasive squamous cell carcinoma."],
 ...
 dict(topic="Nail unit neoplasms", io=IO, slot="referral",
   ...
   opts=[
     ...
     ["A shave biopsy of the nail plate itself, taken in primary care",
      "The nail plate is dead keratin and contains no tumour; the melanocytes lie in the matrix beneath it, which is why an experienced operator is needed."],
