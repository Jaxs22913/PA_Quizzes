 ...
 dict(topic="Actinic keratosis", io=IOA, lead="next step",
   ...
   opts=[
     ["Take a shave or punch biopsy, because persistence or recurrence after therapy is a biopsy trigger",
      "Correct — a lesion that recurs after adequate treatment and has become indurated may already have invaded, and only histology distinguishes actinic keratosis from squamous cell carcinoma."],
 ...
 dict(topic="Actinic keratosis", io=IOA, lead="education",
   ...
   opts=[
     ...
     ["Clearance means the surrounding field is no longer at risk and routine follow-up can stop",
      "The whole sun-damaged field retains dysplastic keratinocytes that were not yet visible, so new lesions continue to arise and surveillance continues."],
 ...
 dict(topic="Squamous cell carcinoma", io=IOA, lead="education",
   ...
   opts=[
     ...
     ["Niacin five hundred milligrams twice daily reduces new squamous cell carcinoma by about thirty per cent",
      "Nicotinamide is the form with evidence here; niacin is a different form of vitamin B3, causes flushing, and is not the agent used for chemoprevention."],
 ...
 dict(topic="Basal cell carcinoma", io=IOA, lead="treatment",
   ...
   opts=[
     ...
     ["Imiquimod five nights weekly for six to ten weeks, with no further review needed if the lesion disappears",
      "A lesion that is no longer visible may still harbour tumour, so clearance has to be confirmed rather than assumed from appearance."],
