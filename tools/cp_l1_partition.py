...
import difflib
from cp_l1_lengthfix import FIXES
for _idx, (_stated, _text) in FIXES.items():
    _q = POOL[_idx]
    _ratio, _oi = max((difflib.SequenceMatcher(None, _text.lower(), _o[0].lower()).ratio(), _i)
                      for _i, _o in enumerate(_q["opts"]))
    assert _oi != _q["c"], "fix %d would rewrite the correct option" % _idx
    _q["opts"][_oi][0] = _text
...
if __name__ == "__main__":
    ...
    json.dump({"set1": s1, "set2": s2}, open(os.path.join(HERE, "cp_l1_sets.json"), "w"),
              ensure_ascii=False, indent=1)
    print("wrote cp_l1_sets.json")
