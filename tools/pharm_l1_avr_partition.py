#!/usr/bin/env python3
"""Partition the Pharmacology I Lecture 1 antiviral pool into two 20s.
...
# SET SIZE: two sets of TWENTY, not thirty.
# The antiviral section is eleven slides covering three drug groups and six
# agents. Forty-four genuinely distinct questions is what that supports; sixty
# would mean asking the same facts twice, which the build spec explicitly
# forbids. Antibacterials (15 classes) and antifungals (5 classes plus fungal
# biology) both carry a full 2x30. Flagged to Jaxon rather than padded.
SET_SIZE = 20
...
import difflib
from pharm_l1_avr_lengthfix import FIXES
...
# The antifungal pool was authored dosage-free from the start, per Dr. Wood.
# Assert it rather than filtering, so a regression is caught rather than hidden.
import re as _re
_DOSE = _re.compile(r"\b\d+(\.\d+)?\s*(mg|milligram|gram|gm|mcg)\b|mg/kg")
assert not [q for q in POOL if _DOSE.search(q["opts"][q["c"]][0])], "dosage recall in pool"
...
if __name__ == "__main__":
    ...
    for _ in range(30000):
        random.shuffle(idx)
        chosen = idx[:SET_SIZE*2]
        s1 = [POOL[i] for i in chosen[:SET_SIZE]]
        s2 = [POOL[i] for i in chosen[SET_SIZE:]]
        total = score(s1) + score(s2)
        if best is None or total < best[0]:
            best = (total, list(chosen))

    chosen = best[1]
    s1 = rotate_for_balance([POOL[i] for i in chosen[:SET_SIZE]])
    s2 = rotate_for_balance([POOL[i] for i in chosen[SET_SIZE:]])
    ...
    json.dump({"set1": s1, "set2": s2},
              open(os.path.join(HERE, "pharm_l1_avr_sets.json"), "w"),
              ensure_ascii=False, indent=1)
    print("wrote pharm_l1_avr_sets.json")
