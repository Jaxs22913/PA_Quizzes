#!/usr/bin/env python3
"""Partition CMS I Lecture 4 Set 1 (Cutaneous Bacterial Infections) into two 30s.
...
Length-bias remediation runs first. This pool came out at 5% raw -- 5 of 100 --
the lowest Set 1 rate so far, against Lecture 2's 66% and Lecture 3's 24%. The
difference is drafting the four options at MATCHED LENGTHS as each question is
written rather than repairing afterwards. Remediation is for the residue.

The lecture carries four objectives, three of them narrow (methicillin-resistant
Staphylococcus aureus, primary versus secondary infection, and care across the
age range), so the scoring weight on covering every objective matters more here
than on a single-objective lecture.
...
if __name__ == "__main__":
    ...
    json.dump({"set1": s1, "set2": s2},
              open(os.path.join(HERE, "cms_l4_set1.json"), "w"), ensure_ascii=False, indent=1)
    print("wrote cms_l4_set1.json")
