...
VIG_B = [
 ...
 dict(topic="Malignant melanoma", io=IOA, lead="next step",
   ...
   opts=[
     ...
     ["Reassure and review in a year, because a lesion under 6 mm cannot be melanoma",
      "The 6 mm criterion is a guide rather than a threshold, and melanomas are detected below it; asymmetry, irregular border and change outweigh size."],
 ...
 dict(topic="Malignant melanoma", io=IOA, lead="test",
   ...
   opts=[
     ...
     ["It should be offered or discussed, since the threshold is 1.0 mm; it is a therapeutic procedure that reliably improves overall survival",
      "The threshold is right but the claim is not: the biopsy stages the disease and guides adjuvant therapy, and removing an involved node has not been shown to prolong survival."],
 ...
 dict(topic="Malignant melanoma", io=IOA, lead="treatment",
   ...
   opts=[
     ...
     ["3 to 5 cm", "In situ disease has not invaded, so 0.5 to 1 cm suffices; margins of several centimetres would cause needless morbidity."]],
   c=0, cite=c(52)),
 ...
 dict(topic="Malignant melanoma", io=IOA, lead="education",
   ...
   opts=[
     ...
     ["Annual self-examination using the recognition mnemonic and ugly-duckling principles, since more frequent checking causes anxiety",
      "Annual checking is too infrequent given how fast melanoma can change; monthly self-examination is what detects recurrence between visits."],
 ...
 dict(topic="Malignant melanoma", io=IOB, lead="education",
   ...
   opts=[
     ...
     ["Lifetime risk is the same in all skin tones, at about 2%",
      "Melanin is substantially protective, so lifetime risk is 0.1 to 0.5 per cent rather than the 2 per cent seen in white individuals, though acral and nail melanoma still occur and are often found late."],
