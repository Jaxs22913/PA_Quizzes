#!/usr/bin/env python3
"""Partition CMS I Lecture 5 Set 1 (Dermatological Infestations) into two 30s.
...
Length-bias remediation runs first. Pool C of this lecture broke the run of low
raw rates -- 48%, 15 of 31 -- because the spider and tick sections are
compare-and-contrast material, where the correct answer is naturally a compound
clause and every wrong-disease distractor a single one. Intending to match
lengths is not enough; the structure the CONTENT imposes has to be checked per
question. Those fifteen were repaired at source in cms_l5_pool_c.py by giving
the distractors the same compound shape.

The lecture carries three objectives: the main compare-and-contrast one, the
primary-versus-secondary lesion distinction, and care across the age range.
...
from cms_l5_pool_d import POOL_D
from cms_l5_lengthfix import SLOT_FIXES
...
if __name__ == "__main__":
    ...
    json.dump({"set1": s1, "set2": s2},
              open(os.path.join(HERE, "cms_l5_set1.json"), "w"), ensure_ascii=False, indent=1)
    print("wrote cms_l5_set1.json")
