#!/usr/bin/env python3
"""Partition CMS I Lecture 6 Set 1 (Cutaneous Viral and Fungal Infections) into two 30s.
...
otherwise." Every question in these five pools is grounded in the deck, and no
lecture audio content will be admitted without checking it against a slide.

LENGTH BIAS. Pool D came in at 79% -- the worst single pool of the derm block --
because its stems ask TWO things ("What is X, and how is it treated?"), which
forces a compound answer that no one-clause distractor can match. Trimming the
answer is the cheap fix elsewhere, but here it stops the option answering the
question, so pool D was repaired by giving every distractor the SAME compound
shape. tools/trim_correct_answers.py now refuses two-part stems for that reason.
All five pools finish at 0%.
...
from cms_l6_pool_a import POOL_A
from cms_l6_pool_b import POOL_B
from cms_l6_pool_c import POOL_C
from cms_l6_pool_d import POOL_D
from cms_l6_pool_e import POOL_E
...
if __name__ == "__main__":
    ...
    json.dump({"set1": s1, "set2": s2},
              open(os.path.join(HERE, "cms_l6_set1.json"), "w"), ensure_ascii=False, indent=1)
    print("wrote cms_l6_set1.json")
