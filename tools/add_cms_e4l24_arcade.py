#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Add the CMS I Exam 4 Lecture 24 (Coronary Artery Disease, Carter) Arcade decks.

Deck content lives in cms_e4l24_arcade_decks.json ({"id","name","color","icon",
"cards":[[prompt, answer], ...]}), slides only (troponin numbers from the
in-class slide 72a), atomic facts, the slide winning over his spoken slips, and
nothing from the unkeyed list (see cms_e4l24_scope.py).
...
"cms-valve-right-prosthetic", i.e. in lecture order before the heart failure decks.
...
CLOSE = "  // <!--/CMSE4-DECKS-->"
DECKS = json.load(open(os.path.join(HERE, "cms_e4l24_arcade_decks.json"), encoding="utf-8"))
...
def main():
    ...
    if not all('"%s"' % i in lst for i in ids):
        anchor = '"cms-valve-right-prosthetic",'
        assert anchor in lst, "exam4 deck list has no cms-valve-right-prosthetic anchor"
