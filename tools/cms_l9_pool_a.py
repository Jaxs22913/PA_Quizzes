...
POOL_A = [
 dict(topic="Clinical approach", io=IOA, slot="manifestation",
   ...
   opts=[
     ["Characterize it systematically — primary lesion type, color, surface texture, border definition, size, distribution, palpability, ulceration, bleeding, induration and temporal evolution",
      "Correct — describe before naming."],
     ["Assign the most likely diagnosis first, then look for the features that confirm it — color, texture, border, size and evolution — and discard those that do not fit",
     ...
     ["Photograph the lesion and defer all description — type, color, texture, border, size, distribution and evolution — until the dermatology referral has been accepted",
      "Description is the first step, not something deferred."],
     ["Biopsy every pigmented lesion before any description of type, color, texture, border or evolution, since morphology is unreliable in premalignant disease",
 ...
 dict(topic="Actinic keratosis", io=IOA, slot="epidemiology",
   ...
   opts=[
     ...
     ["About 1 in 100 lesions per year",
      "One in a hundred per lesion per year would make progression far commoner than observed; the figure is about one in a thousand."],
     ["About 1 in 100,000 lesions per year",
      "One in a hundred thousand understates the risk; the rate is about one in a thousand lesions per year, which is why field treatment is worthwhile."]],
   c=0, cite=c(11)),
 ...
 dict(topic="Actinic keratosis", io=IOA, slot="risk factors",
   ...
   opts=[
     ...
     ["Obesity, diabetes with vascular disease, incontinence and immobility in a hospitalized patient, and recent antibiotic exposure",
 ...
 dict(topic="Actinic keratosis", io=IOA, slot="manifestation",
   ...
   opts=[
     ["Small 0.2 to 0.6 cm flesh-colored, pink or slightly hyperpigmented papules with a characteristic sandpaper texture on palpation",
      "Correct — the sandpaper texture is often felt before it is seen."],
     ["Large 2 to 6 cm deeply pigmented plaques, flesh-colored at the edge, with a smooth waxy stuck-on surface that is more apparent by sight than by touch",
     ...
     ["Small violaceous or purple macules and plaques, flesh-colored early on, that are softer than surrounding skin and more apparent by sight than by touch",
 ...
 dict(topic="Actinic keratosis", io=IOA, slot="manifestation",
   ...
   opts=[
     ["Sun-exposed face, scalp, ears, forearms and dorsal hands; multiple lesions in one zone suggest field cancerization and favor field-directed therapy",
      "Correct — the distribution drives the treatment choice."],
     ["Sun-protected trunk, axillae and groin; multiple lesions in one zone suggest an immunosuppressed host and favor systemic therapy",
      "The distribution is sun-exposed skin."],
     ["Palms, soles and nail beds; multiple lesions in one zone suggest acral disease and favor surgical excision",
      "Acral sites are the melanoma and nail unit pattern."],
     ["Sun-exposed face, scalp and forearms; multiple lesions in one zone suggest a lower risk and favor observation alone",
 ...
 dict(topic="Actinic keratosis", io=IOA, slot="test finding",
   ...
   opts=[
     ...
     ["Flesh-colored or pink color and a size under a centimeter",
 ...
 dict(topic="Actinic keratosis", io=IOA, slot="initial test",
   ...
   opts=[
     ...
     ["By serology for ultraviolet-induced antibodies",
      "No serological test exists for ultraviolet damage; the diagnosis is clinical, supported by dermoscopy in trained hands."]],
   c=0, cite=c(13)),
 ...
 dict(topic="Actinic keratosis", io=IOA, slot="gold standard",
   ...
   opts=[
     ["When morphology or behavior raises concern for squamous cell carcinoma, or the lesion persists or recurs after therapy; the interpretation must separate actinic keratosis from carcinoma in situ from invasive carcinoma",
      "Correct — invasion changes treatment, margins and risk assessment substantially."],
     ["Only after two failed courses of field-directed therapy, whatever the morphology or behavior of the lesion; the interpretation need only confirm that some degree of keratinocytic dysplasia is present, since invasion does not alter treatment or margins",
      "Persistence after therapy is a trigger, but so is initial concern, and the grading matters."],
     ["In every patient over 65 at the first visit regardless of behavior; the interpretation must separate actinic keratosis from seborrheic keratosis and solar lentigo",
