...
topics = [
 {"id": "scope", "label": "How This Exam Is Written", "color": "#69406c", "rows": [
   ["Reference ranges are GIVEN",
    "“I'm not going to just throw a random number at you” — normal ranges are always supplied, on this exam and in every class. Do not spend the night memorizing cutoffs; spend it on what a value means."],
   ["Tests, not diagnoses",
 ...
 {"id": "phases", "label": "The Three Phases", "color": "#8f5f92", "rows": [
   ...
   ["Intratest = analytical", "Performing the test: collection, monitoring the environment, comfort, analgesics and sedatives, vital signs, universal precautions, minimizing delays, watching for complications."],
   ...
   ["Integration & follow-up", "Diagnosis, acceptance, healing, health-promoting behavior. Education, follow-up labs and appointments, referrals, emotional well-being. “If it wasn't documented, it wasn't done.”"],
 ]},
 ...
 {"id": "specimens", "label": "Stool, Blood, Sputum & Throat", "color": "#3f6b5a", "rows": [
   ...
   ["Guaiac", "Heme oxidizes hydrogen peroxide in the guaiac → BLUE = POSITIVE. Use a SMALL sample; a large one obscures the result. Can be done off the gloved finger after digital rectal exam."],
 ...
 {"id": "poct", "label": "Point-of-Care Testing & Regulation", "color": "#2f5b70", "rows": [
   ...
   ["Regulation goes one way", "CLIA is the MINIMUM and cannot be downgraded, so state and city rules are always STRICTER. Every site licensed for ANY testing; license matches complexity; reapply EVERY TWO YEARS."],
 ]},
 {"id": "stats", "label": "Sensitivity, Specificity & Predictive Value", "color": "#8a3f4a", "rows": [
   ...
   ["One dial, two costs", "Move the threshold down to catch every case → false positives. Move it up to exclude cleanly → false negatives. You cannot maximize both, which is why HIV screening is sensitive first, confirmatory testing specific second."],
 ...
 {"id": "modalities", "label": "The Modalities at a Glance", "color": "#3e5f8a", "rows": [
   ...
   ["Computed tomography", "Rotating fan beam, thousands of transmission points. EXPANDS THE GRAY SCALE beyond five densities, reduces overlap, works with implanted devices, 3-D reconstruction. THE CORNERSTONE OF CROSS-SECTIONAL IMAGING. Against: not portable, a LOT of radiation, needs space and processing."],
   ...
   ["Angiographic studies", "NOT one test — any modality can image vessels. X-ray → angiogram. Ultrasound → color Doppler. CT angiography → iodine injected quickly. MR angiography → NO DYE NEEDED. Veins = venogram."],
 ...
 {"id": "density", "label": "Density, Attenuation & Hounsfield", "color": "#7a5a2e", "rows": [
   ...
   ["The WINDOW", "A pre-selected RANGE of Hounsfield numbers (e.g. −100 to +300) spread over the available gray scale. It is a DISPLAY choice, so POST-PROCESSING can re-window the same scan to show different pathology WITHOUT repeating the study or re-exposing the patient."],
 ...
 {"id": "mri-weights", "label": "T1 vs T2 — the always-asked one", "color": "#2f5b70", "rows": [
   ["T2: water is WHITE", "High water content is BRIGHT — fat, edema, infection, blood (hyperemia), cerebrospinal fluid. Low water content is dark gray."],
 ...
 {"id": "positioning", "label": "Positioning & Planes", "color": "#3f6b5a", "rows": [
   ...
   ["Why PA is preferred", "REDUCES MAGNIFICATION OF THE HEART, so cardiomegaly is not misread (the heart is anterior, so on PA it sits near the detector). Also: lower dose to radiation-sensitive organs, maximum lung visualization, better apices, posterior ribs well seen."],
