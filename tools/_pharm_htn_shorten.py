#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Shorten over-long correct answers in the htn pools (Pharmacology Exam 2).

Copied from _pharm_ent_shorten.py for this build. These pools were written
with the keys short from the first draft, so this reports 0 to shorten; it is
kept so a later edit that lengthens a key has the same repair tool as ENT.
...
import glob as _g
FILES = ['pharm_htn_pool_raas.py', 'pharm_htn_pool_ccb.py', 'pharm_htn_pool_beta.py', 'pharm_htn_pool_other.py']
SEPS = [" — ", "; ", ", and ", ", "]
