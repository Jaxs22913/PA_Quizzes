...
POOL_A = [
 dict(topic="What a chemistry panel is", io=IOA, slot="initial test",
   ...
   opts=[
     ["Metabolites, electrolytes, kidney markers, and in the expanded version liver and protein markers",
      "Correct — the panel measures metabolites, electrolytes, kidney markers and, in the expanded version, liver and protein markers: a snapshot of chemical balance and organ function."],
     ["Red cells, white cells, and platelets with their indices",
      "Red cells, white cells and platelets are the complete blood count; a chemistry panel measures metabolites, electrolytes and kidney markers."],
     ...
     ["Hormone levels from the thyroid, adrenal, and pituitary glands",
      "Thyroid, adrenal and pituitary hormones are not panel components; the panel covers metabolites, electrolytes, kidney markers and liver markers."]],
   c=0, cite=c(5)),
 ...
 dict(topic="What a chemistry panel is", io=IOA, slot="initial test",
   ...
   opts=[
     ...
     ["Oxygen delivery, oxygen consumption, and tissue perfusion",
      "Those come from blood gases and hemodynamic measures."],
 ...
 dict(topic="Basic metabolic panel", io=IOA, slot="initial test",
   ...
   opts=[
     ...
     ["Glucose, magnesium, sodium, potassium, chloride, carbon dioxide, blood urea nitrogen, and creatinine",
      "Magnesium is not one of the eight; the basic panel is glucose, calcium, sodium, potassium, chloride, carbon dioxide, urea nitrogen and creatinine."],
 ...
 dict(topic="Panel groupings", io=IOA, slot="initial test",
   q="Where does calcium sit among the panel components?",
   opts=[
     ["On its own, as the mineral",
      "Correct — the panel groups glucose as fuel, the four electrolytes for acid-base, urea nitrogen and creatinine for the kidney, and calcium alone as the mineral."],
     ["With sodium and potassium as an electrolyte",
      "The electrolyte and acid-base group is sodium, potassium, chloride and bicarbonate; calcium is grouped separately, on its own, as the mineral."],
 ...
 dict(topic="Indications", io=IOG, slot="manifestation",
   q="Which cluster of symptoms is an indication for a chemistry panel?",
   opts=[
     ["Fatigue, weakness, edema, jaundice, confusion, nausea and vomiting",
      "Correct — fatigue, weakness, edema, jaundice, confusion, nausea and vomiting are the symptom indication, alongside screening, medications, monitoring and acute illness."],
 ...
 dict(topic="Indications", io=IOG, slot="initial test",
   ...
   opts=[
     ["Dehydration and a suspected acid-base disorder",
      "Correct — dehydration and a suspected acid-base disorder are the acute illness indications; a urea nitrogen to creatinine ratio above 20 suggests hypovolemia."],
     ...
     ["Headache and a suspected migraine",
      "A suspected migraine is not an indication; under acute illness the chemistry panel is ordered for dehydration and a suspected acid-base disorder."]],
   c=0, cite=c(23)),

 dict(topic="Abbreviations", io=IOA, slot="education",
   q="On a chemistry panel, what does the abbreviation Cr stand for?",
   opts=[
     ["Creatinine", "Correct — on the panel Cr is creatinine, the waste product of muscle creatine metabolism that the kidneys filter, paired with urea nitrogen."],
     ["Chromium", "Cr is the element symbol for chromium, but on a chemistry panel it stands for creatinine, the kidney marker read alongside urea nitrogen."],
     ["Creatine kinase", "Creatine kinase is not one of the panel's components; the panel's Cr is creatinine, a waste product of muscle creatine metabolism."],
     ["C-reactive protein", "C-reactive protein is not one of the panel's components; the kidney markers on the panel are urea nitrogen and creatinine."]],
   c=0, cite=c(6)),

 dict(topic="Abbreviations", io=IOA, slot="education",
   q="On a comprehensive metabolic panel, what does the abbreviation ALP stand for?",
   opts=[
     ["Alkaline phosphatase",
      "Correct — ALP (alkaline phosphatase) is one of the six liver and protein markers the comprehensive panel adds, and it marks cholestasis or bile duct obstruction."],
     ["Aspartate aminotransferase",
      "Aspartate aminotransferase is abbreviated AST; it marks hepatocellular injury but is also found in cardiac and skeletal muscle, kidney and brain."],
     ["Albumin",
      "Albumin is written out on the panel rather than abbreviated; it is a synthetic product of the liver with a half-life of about three weeks."],
     ["Alanine transaminase",
      "Alanine transaminase is abbreviated ALT; it is found primarily in the liver and is the more liver-specific marker of injury."]],
   c=0, cite=c(6)),

 dict(topic="Fishbone diagram", io=IOA, slot="education",
   q="The fishbone shorthand diagram carries reference ranges. What does it NOT carry?",
   opts=[
     ["The units", "Correct — the fishbone lays out each value with its reference range, such as sodium 135 to 145, but without the units for those values."],
     ["The reference ranges themselves", "The reference ranges are exactly what the fishbone does carry, written under each analyte; what it leaves out is the units."],
     ["The patient's name and date", "The fishbone is a shorthand layout for laboratory values; the element it visibly omits is the unit attached to each range."],
     ["The abnormal flags", "Abnormal flags are not the point here; the fishbone gives each analyte with its reference range but leaves the units off."]],
   c=0, cite=ci(4)),
 ...
 dict(topic="Fishbone diagram", io=IOA, slot="education",
   ...
   opts=[
     ["Position on the diagram already identifies which test the number belongs to",
      "Correct — the layout itself carries the labeling, which is the point of the shorthand."],
     ["The labels are added later by the laboratory",
      "The diagram is written at the bedside by the clinician."],
     ["Only abnormal values are labeled",
      "Labeling does not depend on whether a value is abnormal."],
     ["The units serve as the labels",
      "The units are the part the diagram omits."]],
   c=0, cite=au()),

 dict(topic="Reference ranges", io=IOA, slot="education",
   q="Why do roughly 2.5 percent of healthy people fall outside a normal range?",
   opts=[
     ...
     ["Laboratory equipment has a known error rate of about 2.5 percent",
      "The figure comes from how the range is defined, not from instrument error."],
     ["About 2.5 percent of samples are mishandled before analysis",
      "Mishandling does produce spurious results, but the 2.5 percent figure follows arithmetically from defining the range as two standard deviations about the mean."],
     ["Roughly 2.5 percent of people have undiagnosed disease",
      "The point is the opposite — these people are healthy."]],
   c=0, cite=c(18)),

 dict(topic="Reference ranges", io=IOA, slot="education",
   q="Following from how a normal range is defined, what does a normal result mean?",
   opts=[
 ...
 dict(topic="Reference ranges", io=IOA, slot="education",
   q="What comes before an extensive workup on a borderline abnormal result?",
   opts=[
     ["Repeat or confirm the abnormality",
      "Correct — normal ranges are the mean plus or minus two standard deviations, so some healthy people fall outside them by chance; confirm before an extensive workup."],
     ...
     ["Treat empirically and recheck afterwards",
      "Treating first inverts the order; a borderline abnormality is repeated or confirmed, as with GGT (gamma-glutamyl transferase) for a raised alkaline phosphatase."],
     ["Refer to the relevant specialist",
      "Referral is not the step for a borderline value; the abnormality is repeated or confirmed first, and every value is read in its clinical context."]],
   c=0, cite=c(18)),
 ...
 dict(topic="Reference ranges", io=IOA, slot="test finding",
   ...
   opts=[
     ...
     ["Using a repeat potassium to confirm a raised potassium is not hemolyzed",
      "Repeating a potassium to exclude hemolysis is sound practice, but the worked confirmation example pairs alkaline phosphatase with gamma-glutamyl transferase."],
