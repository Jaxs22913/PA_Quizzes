#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Split and guard the four Pharmacology Exam 2 antihypertensive pools (Lecture 6).

Copied from pharm_ent_partition.py, with four additions this lecture needed:
  ...
  3. A DESCOPED guard for the caps said aloud and the deck's own errors: nothing
     on AT2, prodrug exceptions or elimination routes, membrane-stabilizing or
     intrinsic sympathomimetic columns, carteolol or betaxolol, the macula densa
     direction (slide 7 prints it without its arrow), "category X". And a KEY
     may never call clonidine an alpha-2 blocker (slide 75's error; slides 86
     and 92 say agonist).
  4. The split is STRATIFIED by topic rather than a plain shuffle, so both
    ...
    RAAS inhibitors and first-line choice  36 -> 2 x 18
    calcium channel blockers                32 -> 2 x 16
    beta blockers                           28 -> 2 x 14
    alpha-1, central, direct vasodilators   33 -> 17 + 16
...
from pharm_htn_pool_raas import QUESTIONS as RAAS
from pharm_htn_pool_ccb import QUESTIONS as CCB
from pharm_htn_pool_beta import QUESTIONS as BETA
from pharm_htn_pool_other import QUESTIONS as OTHER
...
DESCOPED = re.compile(r"(?i)\bAT2\b|\bprodrug|fosinopril|elimination route|renal and bil"
                      r"|membrane[- ]stabiliz|intrinsic sympathomimetic|\bcarteolol\b"
                      r"|\bbetaxolol\b|macula densa|category x|plus signs?")
BAD_KEY = re.compile(r"(?i)alpha-2 (blocker|antagonist)|blocks? (central )?alpha-2")
...
def check(pool, label):
    ...
    for i, q in enumerate(pool):
        ...
        assert not BAD_KEY.search(q["opts"][0][0]), "%s: key repeats the deck's alpha-2 error" % w
        ...
        assert re.match(r"^Antihypertensives\.pptx, Slide \d+$", q["cite"]), "%s: cite %r" % (w, q["cite"])
...
def main():
    ...
    sets = {}
    for label, pool in (("raas", RAAS), ("ccb", CCB), ("beta", BETA), ("other", OTHER)):
        check(pool, label)
        a, b = split(pool, rng)
    ...
    out = os.path.join(HERE, "pharm_htn_sets.json")
