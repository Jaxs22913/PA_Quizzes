#!/usr/bin/env python3
"""Partition the Microbiology Lecture 6 pool into two 30s.

DECK-ONLY. Lecture 6 was delivered on 11 September but no recording exists --
not in the inbox and not in Notability's cache, where Lecture 5's was eventually
found. The audio will inform the guide when it arrives; nothing here depends on
it, because the slides are authoritative anyway.

TWO OBJECTIVES ARE THINLY SERVED BY THE DECK, and the pool reflects that rather
than inventing content: laboratory tests in immunology appear only as antibody
titres and the BAU unit, and the classical-against-alternative complement
comparison belongs to Lecture 5 and survives here only as complement fixation
by antibody.
...
from micro_l6_pool_a import POOL_A
from micro_l6_pool_b import POOL_B
...
if __name__ == "__main__":
    ...
    json.dump({"set1": s1, "set2": s2}, open(os.path.join(HERE, "micro_l6_sets.json"), "w"),
              ensure_ascii=False, indent=1)
    print("\nwrote micro_l6_sets.json")
