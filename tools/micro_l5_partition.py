#!/usr/bin/env python3
"""Partition the Microbiology Lecture 5 pool into two 30s.

The deck runs 93 slides against ELEVEN instructional objectives -- more than
twice the objective count of Lectures 1 to 4 -- so the selection has to spread
across all eleven rather than letting the big topics (complement, natural killer
cells) crowd out the small ones. Asserted below: every set covers at least nine
of the eleven.
...
from micro_l5_pool_a import POOL_A
from micro_l5_pool_b import POOL_B
from micro_l5_pool_c import POOL_C
...
if __name__ == "__main__":
    ...
    json.dump({"set1": s1, "set2": s2}, open(os.path.join(HERE, "micro_l5_sets.json"), "w"),
              ensure_ascii=False, indent=1)
    print("\nwrote micro_l5_sets.json")
