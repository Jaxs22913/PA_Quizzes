...
POOL_B = [
 ...
 dict(topic="Nuclear medicine", io=IOB, slot="agent/regimen",
   ...
   opts=[
     ["Fluorodeoxyglucose-18, a radioactive glucose, because tumors have a higher metabolic rate",
      "Correct — the scan asks which tissues are consuming more glucose."],
     ["Technetium-99, a metastable isotope, because tumors retain it far longer than normal tissue",
      "Technetium-99 is the usual single photon emission tomography agent, tracking blood flow."],
     ["Iodixanol, an iso-osmolal agent, because tumors take up iodine more avidly than muscle",
      "Iodixanol is an intravascular iodinated contrast for angiography, not a nuclear tracer."],
     ["Gadolinium, a paramagnetic metal, because tumors disrupt the local magnetic environment",
 ...
 dict(topic="Nuclear medicine", io=IOB, slot="differential",
   ...
   opts=[
     ["Single photons on rotating gamma cameras, giving three-dimensional images of where blood flows",
      "Correct — a gamma camera rotating around the patient collects single photons from many angles, which reconstruct into a three-dimensional map of where the tracer, and therefore the blood, has gone."],
 ...
 dict(topic="Nuclear medicine", io=IOB, slot="manifestation",
   ...
   opts=[
     ["A particle of roughly the same mass as an electron, but carrying the opposite, positive charge",
      "Correct — it is the electron's antiparticle, and when it meets an electron the two annihilate into paired photons traveling in opposite directions, which is what the scanner detects."],
     ...
     ["An isotope of roughly the same behavior as technetium, but with a much shorter half-life",
 ...
 dict(topic="Nuclear medicine", io=IOF, slot="avoid",
   ...
   opts=[
     ["Computed tomography, positron emission tomography and single photon emission tomography",
      "Correct — all three deliver far more ionizing radiation than plain films, computed tomography through many projections and the nuclear studies through an injected emitter."],
     ["Conventional radiography, fluoroscopy and the angiographic studies performed with x-rays",
      "These use ionizing radiation but at relatively low dose; the highest exposures come from computed tomography and the two nuclear studies."],
     ...
     ["Ultrasonography, color Doppler ultrasonography and the hand-held bedside ultrasound units",
 ...
 dict(topic="Nuclear medicine", io=IOF, slot="education",
   ...
   opts=[
     ...
     ["The exposure is delivered entirely in a single pulse rather than over a continuous scan",
      "The distinction is not timing: in nuclear medicine the radiation source is injected, so the patient continues to emit it after leaving the scanner."],
 ...
 dict(topic="Ultrasonography", io=IOB, slot="first-line",
   ...
   opts=[
     ...
     ["Assessment of metabolic activity, naming tumor staging, brain function and perfusion",
 ...
 dict(topic="Ultrasonography", io=IOB, slot="test finding",
   q="What does the color Doppler setting add to a standard ultrasound image?",
   opts=[
     ["Direction of flow and velocity, on an image otherwise made of white, gray and black",
      "Correct — color is the flow overlay, and blood is what it is used for."],
     ["Depth of penetration and resolution, on an image otherwise limited to superficial planes",
      "Color Doppler does not extend depth; poor deep visualization remains a limitation."],
 ...
 dict(topic="Ultrasonography", io=IOC, slot="initial test",
   ...
   opts=[
     ["Female pelvis and pediatric patients, and it is also used to guide procedures",
     ...
     ["Head-injured and intoxicated patients, and it is also used to exclude a bleed",
      "Intracranial hemorrhage is a computed tomography question in the adult skull."],
 ...
 dict(topic="Ultrasonography", io=IOH, slot="prognosis",
   q="How is ultrasound's safety summarized?",
   opts=[
 ...
 dict(topic="Magnetic resonance imaging", io=IOB, slot="manifestation",
   ...
   opts=[
     ...
     ["The strength of the magnet in Tesla, together with the duration of the scan",
      "Field strength and scan time affect image quality; the frequency emitted depends on the chemical surroundings of each hydrogen atom and where it sits."],
 ...
 dict(topic="Magnetic resonance imaging", io=IOB, slot="test finding",
   ...
   opts=[
     ["Bright, and the examples given are fat, edema, infection, blood and cerebrospinal fluid",
      "Correct — remember T2 as the sequence where water is white."],
     ["Dark, and the examples given are fat, edema, infection, blood and cerebrospinal fluid",
 ...
 dict(topic="Magnetic resonance imaging", io=IOC, slot="first-line",
   ...
   opts=[
     ["Soft tissue, essentially anything other than bone, with extremely high anatomical detail",
      "Correct — soft tissues differ in water and fat content, which is exactly what hydrogen signal detects, so contrast between them is far better than with x-ray attenuation."],
     ["Bone and calcified structures, essentially anything mineralized, with high spatial detail",
     ...
     ["Metabolically active tissue, essentially tumor and infection, with high functional detail",
      "Metabolic activity is what the nuclear medicine studies are indicated for."]],
   c=0, cite=c(27)),
 ...
 dict(topic="Magnetic resonance imaging", io=IOC, slot="first-line",
   ...
   opts=[
     ...
     ["Gadolinium-enhanced imaging, which highlights vascular structures after injection",
      "Contrast enhancement is used for tumors and masses rather than for stroke here."]],
   c=0, cite=c(30)),
 ...
 dict(topic="Magnetic resonance imaging", io=IOC, slot="first-line",
   ...
   opts=[
     ["Neuro-imaging, and the soft tissues of orthopedics such as muscle, ligament and tendon",
 ...
 dict(topic="Angiographic studies", io=IOB, slot="manifestation",
   ...
   opts=[
     ...
     ["A specific ultrasound study in which the transducer is placed within the vessel",
      "The term is not tied to one technique or to an intravascular probe; any of the modalities can be used to image vessels."]],
   c=0, cite=c(32)),
 ...
 dict(topic="Angiographic studies", io=IOB, slot="manifestation",
   ...
   opts=[
     ...
     ["Doppler study, performed as either a color Doppler examination or a duplex examination",
      "Color Doppler shows flow in either vessel type; it is not the venous-specific term."],
     ["Coronary angiogram, performed as either a diagnostic catheterization or an intervention",
 ...
 dict(topic="Fluoroscopy", io=IOB, slot="manifestation",
   ...
   opts=[
     ["Motion of body parts and positioning changes of bones and joints, viewed in real time",
      "Correct — real-time visualization is the point of the study."],
     ...
     ["Water content in soft tissue and edema, viewed on differently weighted sequences",
      "Weighted sequences and water content belong to magnetic resonance imaging."],
     ["Direction and velocity of blood flow, viewed as color superimposed on gray scale",
      "Color flow mapping is the Doppler setting on an ultrasound machine."]],
   c=0, cite=c(57)),
]
