...
VIG_B = [
 ...
 dict(topic="Ectropion", io=IOA, lead="first-line", slot="first-line",
   ...
   opts=[
     ["Preservative-free artificial tears during the day, lubricating ointment at night, and taping the lid into place",
      "Correct — the everted lid leaves the cornea and conjunctiva exposed, so lubrication protects the surface and taping holds the lid in position until surgery."],
 ...
 dict(topic="Ectropion", io=IOA, lead="education", slot="escalation",
   ...
   opts=[
     ["Surgery is the definitive treatment; lubrication only protects the surface in the meantime",
      "Correct — the lid malposition is mechanical, so only surgical repositioning corrects it; lubrication manages the consequences."],
 ...
 dict(topic="Dermatochalasis", io=IOA, lead="initial test", slot="initial test",
   ...
   opts=[
     ["Visual field testing",
      "Correct — blepharoplasty is treated as cosmetic unless a visual field deficit is documented, so the field test establishes functional need."],
 ...
 dict(topic="Xanthelasma", io=IOA, lead="next step", slot="initial test",
   ...
   opts=[
     ...
     ["Serum lipid profile alone",
      "A lipid profile alone is incomplete; diabetes mellitus and liver dysfunction also underlie the lesion."],
     ...
     ["No workup — the lesions are purely cosmetic",
      "They are not merely cosmetic; they signal a metabolic disorder carrying cardiovascular risk, and treatment is aimed at that."]],
   c=0, cite=c(17)),
 ...
 dict(topic="Xanthelasma", io=IOA, lead="education", slot="prognosis",
   ...
   opts=[
     ["Even after effective local treatment, recurrences are common",
      "Correct — local removal clears the deposit but not the metabolic driver behind it, so lesions commonly return."],
     ...
     ["The lesions will resolve on their own if left alone",
      "These plaques do not resolve spontaneously; they persist and often enlarge while the metabolic driver remains."],
 ...
 dict(topic="Pterygium", io=IOA, lead="next step", slot="referral",
   ...
   opts=[
     ["Non-urgent referral to ophthalmology for slit lamp assessment of the adjacent cornea",
      "Correct — documented growth and reported visual impairment are the two triggers for referral, and a lesion that grows over a year is not an emergency."],
     ["Emergent same-day referral",
      "A pterygium advances over months to years rather than hours, so referral is non-urgent."],
     ...
     ["Start a topical corticosteroid",
      "Corticosteroids do not reverse fibrovascular tissue already grown onto the cornea, and prolonged use risks raised pressure and cataract."]],
   c=0, cite=c(28)),
 ...
 dict(topic="Pterygium", io=IOA, lead="education", slot="education",
   ...
   opts=[
     ["Conservative management may control symptoms but will not make the lesion resolve",
      "Correct — protection and lubrication relieve irritation and slow progression, but the degenerative tissue already laid down does not regress."],
     ["Drops and sun protection will clear it within a few months",
      "Conservative measures ease symptoms but do not remove deposited fibrovascular tissue."],
     ["It will resolve on its own once sun exposure stops",
      "Stopping exposure slows further growth but does not reverse what has already formed."],
     ["Only oral medication will shrink it",
      "No oral agent shrinks a pterygium; the only definitive removal is surgical."]],
   c=0, cite=c(28)),
 ...
 dict(topic="Chemosis", io=IOA, lead="next step", slot="referral",
   ...
   opts=[
     ["Treat this as urgent — proptosis and restricted movement alongside chemosis are red flags",
      "Correct — proptosis and restricted movement mean the orbit is involved rather than the conjunctiva alone, raising orbital cellulitis or a mass."],
 ...
 dict(topic="Autoimmune conjunctivitis", io=IOA, lead="next step", slot="referral",
   ...
   opts=[
     ["Routine referral to ophthalmology",
      "Correct — autoimmune conjunctivitis is chronic and not sight-threatening in the short term, so it is referred routinely rather than urgently."],
 ...
 dict(topic="Red eye triage", io=IOA, lead="next step", slot="initial test",
   ...
   opts=[
     ["Visual acuity in each eye with correction, pupils, extraocular movements, corneal clarity with fluorescein, the pattern of injection and discharge, and the contact lens, trauma, surgery and steroid history",
      "Correct — acuity, pupils, movements and corneal clarity are what separate a benign red eye from a sight-threatening one, and the history identifies the highest-risk patients."],
     ["Computed tomography of the orbits with contrast",
      "Imaging is not the first step in a red eye."],
     ["A trial of topical antibiotic for 48 hours",
      "Treating before examining risks giving an antibiotic to a corneal ulcer or acute glaucoma, where delay costs vision."],
 ...
 dict(topic="Red eye triage", io=IOA, lead="next step", slot="escalation",
   ...
   opts=[
     ...
     ["Obtain the full history first, then irrigate",
      "Alkali keeps penetrating while the history is taken, so irrigation starts immediately and the history follows."],
 ...
 dict(topic="Red eye triage", io=IOA, lead="next step", slot="referral",
   ...
   opts=[
     ["Routine follow-up", "Correct — these findings together exclude every sight-threatening cause, and reliable follow-up covers the possibility of later change."],
 ...
 dict(topic="Fluorescein angiography", io=IOA, lead="education", slot="education",
   ...
   opts=[
     ["Fluorescein contains no iodine and is relatively safe",
      "Correct — fluorescein contains no iodine, so a reaction to iodinated radiographic contrast does not predict a reaction to it."],
 ...
 dict(topic="Bacterial conjunctivitis", io=IOA, lead="education", slot="education",
   ...
   opts=[
     ["Take contagious precautions with careful hand hygiene, and expect progressive improvement within days",
      "Correct — bacterial conjunctivitis spreads by hand and shared surfaces, and an appropriate topical antibiotic should produce visible improvement within days."],
     ["No precautions are needed as bacterial conjunctivitis is not contagious",
      "Bacterial conjunctivitis is transmissible by direct contact, so hand hygiene and avoiding shared towels matter."],
 ...
 dict(topic="Anterior uveitis", io=IOA, lead="next step", slot="escalation",
   ...
   opts=[
     ["Thorough systemic evaluation for an underlying autoimmune condition",
      "Correct — recurrent uveitis with axial joint symptoms points to an HLA-B27 associated spondyloarthropathy, where the same process inflames joints and uveal tract."],
 ...
 dict(topic="Keratitis", io=IOA, lead="prognosis", slot="prognosis",
   ...
   opts=[
     ["The prognosis is worse, because ulcers inside the visual axis carry a poorer outcome",
      "Correct — healing leaves a scar, and a scar in the visual axis obstructs the line of sight however small it is."],
     ["Position makes no difference; only ulcer size matters",
      "Size matters, but so does position: a scar directly in the visual axis degrades vision even when the ulcer is modest."],
     ["The prognosis is better, because central ulcers are noticed sooner",
      "Earlier recognition helps, but the residual scar still sits across the line of sight, so central ulcers do worse."],
     ["The prognosis depends only on the organism",
      "The organism matters for treatment, but outcome also tracks how much cornea is destroyed and where."]],
   c=0, cite=c(56)),
 ...
 dict(topic="Trachoma", io=IOA, lead="next step", slot="escalation",
   ...
   opts=[
     ["Surgical treatment of the trichiasis",
      "Correct — the inturned lashes result from fixed cicatricial scarring, which only lid surgery can correct before the cornea is destroyed."],
     ...
     ["Observation with annual review",
      "Observation allows the lashes to keep abrading the cornea towards blindness; lid surgery is needed."]],
   c=0, cite=c(46)),
 ...
 dict(topic="Blepharitis", io=IOA, lead="next step", slot="escalation",
   ...
   opts=[
     ...
     ["Start a topical corticosteroid",
      "Topical corticosteroids are not part of the escalation and carry pressure and cataract risk with prolonged use."]],
   c=0, cite=c(19)),
 ...
 dict(topic="Hordeolum with cellulitis", io=IOA, lead="next step", slot="escalation",
   ...
   opts=[
     ["Treat as pre-septal cellulitis with systemic antibiotics",
      "Correct — the infection has spread from the lid gland into surrounding tissue but remains in front of the septum, since the globe is white and movements painless."],
